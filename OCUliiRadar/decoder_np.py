...
class HandshakePacket(object):
  def __init__(self, idx_packet, timestamp_packet_receive, bytes_stream):
    ...
    # assert delay >= 0
    if delay >= 0:
      log('    delay:{:.9f} s'.format(delay))
    else:
      log_YELLOW('    delay:{:.9f} s'.format(delay))
...
class OCULiiDecoderNetworkPackets(object):
      ...
      while True:
        try:
          ...
          if not (ip_src_legal and ip_dst_legal):
            print('[{:.6f}] idx_packet={}, ip illegal, skip'.format(ts, idx_packet))
            continue
    ...
    def generate_frame(self):
      ...
      if hasattr(body_pkg, 'pts'):
        ...
        log_RED('    ({})Do not have pts, do not generate {}'.format(len(self.frame_infos), pcd_filename))
