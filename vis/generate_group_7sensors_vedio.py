    ...
    for idx, item in enumerate(os.listdir(root_dataset)):
        ...
        with writer.saving(fig, output_path, dpi=dpi_vedio):
            for id_frame, name_frame_folder in enumerate(names_frame_folder):
                ...
                '''
                visualize and plot
                row1: IRayCamera, LeopardCamera0, LeopardCamera1, MEMS
                ...
                MEMS_height, MEMS_width = IRayCamera_png_height, IRayCamera_png_width

                w_all = IRayCamera_png_width + LeopardCamera0_png_width + LeopardCamera1_png_width\
                        + MEMS_width + (n_col_row1 + 1) * w_padding_row1
                ...
                h_row1 = np.array([IRayCamera_png_height, LeopardCamera0_png_height, LeopardCamera1_png_height, MEMS_height]).max()
                ...
                # MEMS
                left4 = left3 + w3 + w_padding_row1 / w_all
                bottom4 = (h_padding_mid + h_row2 + h_padding_down) / h_all
                w4 = MEMS_width / w_all
                h4 = h_row1 / h_all
                ax4 = fig.add_axes([left4, bottom4, w4, h4], projection=ccrs.PlateCarree())
                ax4.plot(route['longitude_E'].values, route['latitude_N'].values, '-', c='k', linewidth=1)
                ax4.scatter(route['longitude_E'].values, route['latitude_N'].values, s=10, c=np.round(route.index.values), cmap='jet')
                ax4.scatter(MEMS_json['msg_ins']['longitude_E'], MEMS_json['msg_ins']['latitude_N'], s=100, c='red', marker='P')
                delta_latitude_N = route['latitude_N'].values[0] - route['latitude_N'].values[-1]
                delta_longitude_E = route['longitude_E'].values[0] - route['longitude_E'].values[-1]
                if delta_latitude_N * delta_longitude_E > 0:
                    ax4.text(
                        route['longitude_E'].values.min(), route['latitude_N'].values.max(),
                        '{:.6f}N\n{:.6f}E\nvn={:.3f}m/s\nve={:.3f}m/s\nazimuth={:.3f}°'.format(
                            MEMS_json['msg_ins']['longitude_E'], MEMS_json['msg_ins']['latitude_N'],
                            MEMS_json['msg_ins']['north_vel'], MEMS_json['msg_ins']['east_vel'],
                            MEMS_json['msg_ins']['azimuth']
                        ),
                        horizontalalignment='left', verticalalignment='top'
                    )
                else:
                    ax4.text(
                        route['longitude_E'].values.min(), route['latitude_N'].values.min(),
                        '{:.6f}°N\n{:.6f}°E\nvn={:.3f}m/s\nve={:.3f}m/s\nazimuth={:.3f}°'.format(
                            MEMS_json['msg_ins']['longitude_E'], MEMS_json['msg_ins']['latitude_N'],
                            MEMS_json['msg_ins']['north_vel'], MEMS_json['msg_ins']['east_vel'],
                            MEMS_json['msg_ins']['azimuth']
                ...
                ax4.set_aspect(MEMS_height/MEMS_width*delta_x/delta_y)
                ax4.set_title('MEMS({}/{})\n{}'.format(id_frame + 1, num_frames, MEMS_localtime))
